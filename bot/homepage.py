...
def update_homepage(jobs=None):
    ...
    # Production compatibility: html_generator passes the current
    # merged jobs list. Rebuild sections from that exact dataset so
    # stale in-memory sections cannot keep old homepage entries.
    if jobs is not None:
...
def sort_jobs(jobs):
    ...
    scraped_at is the primary signal because it records when the
    automation actually collected the job. publish_date/date is only
    a fallback when scraped_at is unavailable.
    """
    ...
    def sort_key(job):
        ...
        primary = scraped or published or datetime.min

        return (
            primary,
            published or datetime.min,
