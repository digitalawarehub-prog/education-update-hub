...
def build_homepage_card(job):
    """
    Latest Updates title-only item.

    No image, category badge, date, or Read More button.
    Only the clickable post title is generated.
    """
...
def update_homepage(jobs=None):
    """
    Update homepage using the supplied jobs dataset.

    Production compatibility:
    - scraper.py calls update_homepage(jobs)
    - if jobs are supplied, rebuild sections from the latest sorted jobs
    - if jobs are not supplied, reuse the already registered sections
    """
    ...
    if jobs is not None:
        logger.info(
            "Homepage rebuilt from supplied jobs: %d",
            len(jobs)
        )
...
def _parse_job_datetime(value):
    """
    Convert common job date/timestamp formats into a sortable datetime.
...
def sort_jobs(jobs):
    ...
    Priority:
    1. publish_date when valid
    2. scraped_at when publish_date is missing/invalid
    3. datetime.min as final fallback

    scraped_at is used as a tie-breaker.
    """

    def sort_key(job):

        published = _parse_job_datetime(
            job.get("publish_date")
            or job.get("date")
        )

        scraped = _parse_job_datetime(
            job.get("scraped_at")
        )

        primary = published or scraped or datetime.min
        tie_breaker = scraped or datetime.min

        return (
            primary,
            tie_breaker,
            safe(job.get("title")).lower()
        )
