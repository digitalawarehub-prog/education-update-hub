...
def build_html_body(job):
    ...
    # ======================================================
    # CATEGORY-WISE PRIMARY ACTION BUTTON
    ...
    # ======================================================
    raw_category = str(job.get("category", "Latest Jobs") or "").strip()
    category_key = raw_category.lower().replace("_", " ").replace("-", " ")

    action_map = {
        "recruitment": (job.get("apply_link") or "", "🚀 ऑनलाइन आवेदन करें", "apply-btn"),
        "latest jobs": (job.get("apply_link") or "", "🚀 ऑनलाइन आवेदन करें", "apply-btn"),
        "admit card": (job.get("admit_card_url") or "", "🎫 प्रवेश पत्र डाउनलोड करें", "apply-btn"),
        "result": (job.get("result_url") or "", "📊 परिणाम देखें", "apply-btn"),
        "results": (job.get("result_url") or "", "📊 परिणाम देखें", "apply-btn"),
        "answer key": (job.get("answer_key_url") or "", "📄 उत्तर कुंजी देखें", "apply-btn"),
        "syllabus": (job.get("syllabus_url") or "", "📚 पाठ्यक्रम देखें", "apply-btn"),
    }

    action_link, action_label, action_class = action_map.get(
        category_key, (job.get("apply_link") or "", "🚀 ऑनलाइन आवेदन करें", "apply-btn")
    )
...
{f'<a class="{action_class}" href="{action_link}" target="_blank">{action_label}</a>' if action_link else ''}

{f'<a class="notification-btn" href="{notification}" target="_blank">📄 आधिकारिक अधिसूचना डाउनलोड करें</a>' if notification else ''}

{f'<a class="official-btn" href="{official}" target="_blank">🌐 आधिकारिक वेबसाइट</a>' if official else ''}
