...
def _action_type(job):
    """Detect the correct action type from category/title."""
    ...
    if category in ("recruitment", "latest jobs", "central jobs",
                    "uttarakhand jobs", "other state jobs"):
        return "recruitment"

    if any(x in text for x in (
        "recruitment", "vacancy", "advertisement", "advt",
        "apply online", "online application", "walk in", "भर्ती"
    )):
        return "recruitment"
...
def action_button_data(job):
    """
    Return category-specific button text and URL.
    Recruitment NEVER falls back to notification_pdf.
    """
    ...
    if action == "admit":
        ...
        # Critical fix: never use the notification PDF as Apply URL.
        link = apply_link or official or "#"
...
def build_html_body(job):
    ...
    action_label, action_link = action_button_data(job)
