...
def generate_slug(title):
    """Generate a stable slug. Never return an empty or random slug."""
    if not title:
        return "post"
    ...
    # Python's built-in hash() is randomized between runs, which can
    # change a post URL every automation run. Use deterministic SHA-1.
...
def generate_sitemap(jobs):
    """Rebuild sitemap.xml on every automation run."""
    urls = []
    seen = set()
    ...
    for path in STATIC_SITEMAP_PATHS:
        ...
        if loc not in seen:
            seen.add(loc)
            urls.append((loc, datetime.utcnow().strftime("%Y-%m-%d")))
    ...
    for job in jobs:
        ...
        dt = job_datetime(job)
        lastmod = dt.strftime("%Y-%m-%d") if dt != datetime.min else datetime.utcnow().strftime("%Y-%m-%d")
        seen.add(loc)
    ...
    logger.info("Sitemap Updated : %d URLs", len(urls))
    return True
