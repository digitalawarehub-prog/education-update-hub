...
def detail_value(job, key, default):
    aliases = {
        "vacancy": ("vacancy", "posts", "no_of_posts", "number_of_posts"),
        "qualification": ("qualification", "eligibility", "educational_qualification"),
        "salary": ("salary", "pay_scale", "pay", "pay_level"),
        "last_date": ("last_date", "closing_date", "application_last_date"),
    }
    ...
    patterns = {
        ...
        "qualification": [
            r"(?:educational\s+qualification|essential\s+qualification|qualification|eligibility)\s*(?:[:\-]|is|are)?\s*([^\n|.;]{5,250})",
        ],
        "salary": [
            r"(?:salary|pay\s+scale|pay\s+level|remuneration|emoluments?)\s*(?:[:\-]|is|are)?\s*([^\n|.;]{2,180})",
        ],
        "last_date": [
            r"(?:last\s+date|closing\s+date|application\s+last\s+date|apply\s+online\s+last\s+date)\s*(?:[:\-]|is|are)?\s*([^\n|]{3,120})",
        ],
    }
    return _extract_detail_from_text(text, patterns.get(key, [])) or default
...
def build_html_body(job):
    ...
    vacancy = escape_html(
        detail_value(job, "vacancy", "Check Official Notification")
    )

    qualification = escape_html(
        detail_value(job, "qualification", "Check Official Notification")
    )

    salary = escape_html(
        detail_value(job, "salary", "Check Official Notification")
    )

    last_date = escape_html(
        detail_value(job, "last_date", "Check Official Notification")
    )
