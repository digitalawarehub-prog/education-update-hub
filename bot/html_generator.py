...
.post-buttons {{display:flex;flex-wrap:wrap;gap:12px;margin:28px 0}}
.post-buttons a {{display:inline-flex;align-items:center;justify-content:center;min-height:46px;padding:11px 16px;border-radius:8px;text-decoration:none;font-weight:700;color:#fff}}
.post-buttons .apply-btn {{background:#1769e0}}
.post-buttons .notification-btn {{background:#475569}}
.post-buttons .vacancy-btn {{background:#0f766e}}
.post-buttons .admit-btn {{background:#7b3fb6}}
.post-buttons .result-btn {{background:#16865b}}
.post-buttons .answer-key-btn {{background:#d97706}}
.post-buttons .syllabus-btn {{background:#0f766e}}
.post-buttons .official-btn {{background:#334155}}
@media(max-width:700px) {{.post-buttons a {{width:100%}}}}
</style>
</head>
"""
# ==========================================================
...
def _http_link(value):
    value=str(value or "").strip()
    return value if re.match(r"^https?://",value,re.I) else ""

def build_post_action_buttons(job):
    raw_cat=str(job.get("category","") or "").strip().lower(); title=str(job.get("title","") or "").lower(); text=raw_cat+" "+title
    seen=set(); out=[]
    def add(label, css, *values):
        for v in values:
            href=_http_link(v)
            if href and href not in seen:
                seen.add(href); out.append(f'<a class="{css}" href="{html.escape(href,quote=True)}" target="_blank" rel="noopener">{label}</a>'); return
    if any(x in text for x in ("result","merit list","score card","scorecard","परिणाम")):
        add("📊 रिजल्ट देखें","result-btn",job.get("result_link"),job.get("result_url"),job.get("url"))
    elif any(x in text for x in ("admit card","hall ticket","प्रवेश पत्र")):
        add("🎫 प्रवेश पत्र डाउनलोड करें","admit-btn",job.get("admit_card_link"),job.get("admit_card_url"),job.get("url"))
    elif any(x in text for x in ("answer key","उत्तर कुंजी")):
        add("📄 उत्तर कुंजी देखें","answer-key-btn",job.get("answer_key_link"),job.get("answer_key_url"),job.get("url"))
    elif any(x in text for x in ("syllabus","exam pattern","पाठ्यक्रम")):
        add("📚 सिलेबस देखें","syllabus-btn",job.get("syllabus_link"),job.get("syllabus_url"),job.get("url"))
    else:
        add("🚀 ऑनलाइन आवेदन करें","apply-btn",job.get("apply_link"),job.get("application_link"),job.get("apply_url"))
        if not out: add("🌐 आधिकारिक वेबसाइट देखें","official-btn",job.get("official_website"),job.get("url"))
    add("📄 अधिसूचना देखें","notification-btn",job.get("notification_pdf"),job.get("notification_link"))
    add("📋 भर्ती विवरण देखें","vacancy-btn",job.get("vacancy_link"),job.get("vacancy_url"))
    add("🌐 आधिकारिक वेबसाइट","official-btn",job.get("official_website"))
    return "\n".join(out)
...
def build_html_body(job):
    ...
    action_buttons_html = build_post_action_buttons(job)
...
def cleanup_legacy_generated_posts():
    """Remove auto-generated posts created by the old generic template.

    Older builds used the same generic FAQ and the generic "ऑनलाइन आवेदन करें"
    action button for every category. Those files can survive in generated/posts
    even after the generator code is updated, so remove only files carrying the
    old template markers before the current generator writes fresh HTML.
    """
    legacy_markers = (
        "This page provides complete official information",
        "Click the Apply Online button above",
        "Use the Download Notification button available above",
        "What is {title}?",
    )
    removed = 0
    for path in OUTPUT_DIR.glob("*.html"):
        try:
            ...
            logger.exception("Unable to inspect legacy generated post: %s", path)
    logger.info("LEGACY TEMPLATE CLEANUP | Removed=%d", removed)
    return removed


def generate_all(jobs, category_jobs=None):
    cleanup_legacy_generated_posts()
    all_jobs=list(jobs or []); generated=[]; failed=0; seen=set()
    for job in all_jobs:
        try:
            title=str(job.get("title","")).strip(); cat=str(job.get("category","")).strip().lower()
            if not title or title.lower() in INVALID_TITLES or cat=="unknown": failed+=1; continue
            slug=generate_slug(title,job)
            if slug in seen: continue
            seen.add(slug); filepath=generate_post(job)
            if filepath: generated.append(filepath)
            else: failed+=1
        except Exception:
            logger.exception("Generation Failed : %s",job.get("title","")); failed+=1
    logger.info("Database Jobs : %d | Generated : %d | Failed : %d",len(all_jobs),len(generated),failed)
    try: category_generator.build_categories(all_jobs)
    except Exception: logger.exception("Category Generator Failed")
    return {"success":len(generated),"failed":failed,"total":len(all_jobs),"results":[{"success":True,"file":str(f),"title":Path(f).stem,"slug":Path(f).stem} for f in generated]}
...
def build_site(jobs):
    ...
    result=generate_all(all_jobs)
    verify_generated_files()
    homepage.run(all_jobs)
    html_statistics()
